...
@test "tenant chart writes namespace.cozystack.io/gateway label and _namespace.gateway in lockstep with tenant.gatewayEffective" {
  ...
  # tenant.gatewayEffective helper. If they diverge — auto-default
  ...
  #
  # Provisions a derived-apex child tenant under tenant-test (which
  # has gateway-enabled at the platform level), then asserts:
